...
class Product{
    //constructor 
    constructor(readonly container: HTMLTableSectionElement){}

    //rendering new product
    render(product: string, price: number, quantity: number):void{
    ...
    addPrices(){
        ...
        for(let i = 0; i < rows; i++){
            ...
            let textNode:any = tdElement.childNodes.item(0); //getting the content of each of the 5th cells

            var number:number = parseFloat(textNode.data);  //converting to number
...
function deleteRow(r: any){
    ...
    var tableBody = document.querySelector('table') as HTMLTableElement;
...
//getting the values
const productName = document.getElementById('product') as HTMLInputElement;
const quantity = document.getElementById('quantity') as HTMLInputElement;
const unitPrice = document.getElementById('price') as HTMLInputElement;

//where to display values
const grandTotal = document.getElementById('grandTotal') as HTMLSpanElement;
...
const tableCaptionD = document.querySelector('caption') as HTMLTableCaptionElement;

//form 
const productForm = document.getElementById('form') as HTMLFormElement;
productForm.addEventListener('submit', (e: Event)=>{
    e.preventDefault();

    //formatting values gotten
    let tableValues : [string, number, number];
